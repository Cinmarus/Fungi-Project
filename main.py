...
def main():
    ...
    time = df["timestamp"]  # Replace "timestamp" with the correct column name if different
    ...
    try:
        baseline, offset_signal = extract_baseline_and_offset(
            df["Voltage"],  # Replace "Voltage" with the correct column name if different
    ...
    df["Offset_Signal"] = offset_signal
